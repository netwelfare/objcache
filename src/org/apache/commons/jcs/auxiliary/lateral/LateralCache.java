...
public class LateralCache<K, V> extends AbstractAuxiliaryCacheEventLogging<K, V>
{
	...
	protected void processUpdate(ICacheElement<K, V> ce) throws IOException
	{
		...
		catch (NullPointerException npe)
		{
			...
			handleException(npe,
					"Failed to put [" + ce.getKey() + "] to " + ce.getCacheName() + "@" + lateralCacheAttributes);
			return;
		}
		catch (Exception ex)
		{
	...
	protected ICacheElement<K, V> processGet(K key) throws IOException
	{
		...
		{
			log.error(e);
			handleException(e, "Failed to get [" + key + "] from " + lateralCacheAttributes.getCacheName() + "@"
	...
	protected Map<K, ICacheElement<K, V>> processGetMatching(String pattern) throws IOException
	{
		...
		{
			log.error(e);
			handleException(e, "Failed to getMatching [" + pattern + "] from " + lateralCacheAttributes.getCacheName()
					+ "@" + lateralCacheAttributes);
			return Collections.emptyMap();
		}
	}
	...
	protected boolean processRemove(K key) throws IOException
	{
		...
		{
			handleException(ex, "Failed to remove " + key + " from " + lateralCacheAttributes.getCacheName() + "@"
					+ lateralCacheAttributes);
		}
		return false;
	}
	...
	protected void processRemoveAll() throws IOException
	{
		...
		{
			handleException(ex, "Failed to remove all from " + lateralCacheAttributes.getCacheName() + "@"
					+ lateralCacheAttributes);
		}
	}
