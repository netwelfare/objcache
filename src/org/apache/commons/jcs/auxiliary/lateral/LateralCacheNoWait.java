 ...
 * 仅仅是cache的封装，通过队列，进行了cache的封装，其实也是cache的方法
 */
public class LateralCacheNoWait<K, V> extends AbstractAuxiliaryCache<K, V>
{
	...
	public LateralCacheNoWait(LateralCache<K, V> cache)
	{
		...
		this.eventQueue = fact.createCacheEventQueue(new CacheAdaptor<K, V>(cache), CacheInfo.listenerId,
				cache.getCacheName(), cache.getAuxiliaryCacheAttributes().getEventQueuePoolName(),
				cache.getAuxiliaryCacheAttributes().getEventQueueType());
