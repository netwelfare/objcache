...
public class LateralTCPCacheManager extends LateralCacheAbstractManager
{
	...
	private static final Map<String, LateralTCPCacheManager> instances = new ConcurrentHashMap<String, LateralTCPCacheManager>();
			...
			ICacheEventLogger cacheEventLogger, IElementSerializer elementSerializer)
	{
		...
		catch (Exception ex)
		{
			...
			this.lateralService = new ZombieCacheServiceNonLocal<Serializable, Serializable>(
	...
	public <K, V> LateralCacheNoWait<K, V> getCache(String cacheName)
	{
		...
		synchronized (caches)
		{
			...
			if (lateralNoWait == null)
			{
				...
				if (log.isInfoEnabled())
				{
					log.info("Created LateralCacheNoWait for [" + lateralCacheAttributes + "] LateralCacheNoWait = ["
							+ lateralNoWait + "]");
				}
